...
void DisPlayMenu(SDL_Renderer* renderer, SDL_Event e) {
	...
	SDL_Surface* exit_ = TTF_RenderText_Solid(font, (string("Play")).c_str(), Red);
	SDL_Texture* exit_text = SDL_CreateTextureFromSurface(renderer, exit_);
	SDL_Rect exit_rect;
	exit_rect.w = 300;
	exit_rect.h = 200;
	exit_rect.x = (TILE * TILE - exit_rect.w) / 2;
	exit_rect.y = (TILE * TILE - exit_rect.h) / 2;
	SDL_RenderCopy(renderer, exit_text, NULL, &exit_rect);
	font = TTF_OpenFont("font.ttf", 15);
	SDL_Surface* message = TTF_RenderText_Solid(font, (string("Exit")).c_str(), Yellow);
	...
	message_rect.y = exit_rect.y + exit_rect.h + 50;
	...
	while (true) {
		...
		if (SDL_PollEvent(&event)) {
			...
			if (event.type == SDL_MOUSEBUTTONDOWN 
				&& x>= (TILE * TILE - message_rect.w) / 2 
				&& x<= (TILE * TILE - message_rect.w) / 2 + TILE * TILE - 6 * TILE
				&& y>= exit_rect.y + exit_rect.h + 50
				&& y<= exit_rect.y + exit_rect.h + 50+ 100) {
				exit(0);
			}
			if (event.type == SDL_MOUSEBUTTONDOWN
				&& x >= (TILE * TILE - exit_rect.w) / 2
				&& x <= (TILE * TILE - exit_rect.w) / 2 + 300
				&& y >= (TILE * TILE - exit_rect.h) / 2
				&& y <= (TILE * TILE - exit_rect.h) / 2 + 200)
				return;
		}
	}
}
...
void DisplayGameOver(SDL_Renderer* renderer, SDL_Event e, int tailLength) {
	...
	UpdateHighScore(tailLength * 100);
...
void RenderScrore(SDL_Renderer* renderer, int taillength) {
	...
	SDL_Surface* high_score = TTF_RenderText_Solid(font, (string("HighScore: ") + highScore).c_str(), White);
	SDL_Texture* tex_high_score_ = SDL_CreateTextureFromSurface(renderer, high_score);
	SDL_Rect highscore;
	highscore.w = 100;
	highscore.h = 30;
	highscore.x = 300;
	highscore.y = 0;
	SDL_RenderCopy(renderer, tex_score_, NULL, &score);
	SDL_RenderCopy(renderer, tex_high_score_, NULL, &highscore);
	TTF_CloseFont(font);
}